...
const AddScreen = ({ route }) => {
  ...
  const submitVerbatim = () => {
    ...
    const getVerbastardUsername = () => {
      const verbastardId = route.params.value; // Get the verbastard id
      const verbastardRef = ref(db, `Users/${verbastardId}`);
      return get(verbastardRef).then((snapshot) => {
        if (snapshot.exists()) {
          const verbastardData = snapshot.val();
          return verbastardData.username; // Return the verbastard username
        } else {
          return null;
        }
      });
    ...
    onValue(usersRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (matchingUser) {
          ...
          getVerbastardUsername().then((verbastardUsername) => {
            if (verbastardUsername) {
              const { value } = route.params;
              ...
              set(ref(db, 'Verbatims/' + newVerbatimKey), {
                ...
                post: postText,
                timestamp: formattedDate,
                verbaiter: selectedVerbaiter ? selectedVerbaiter.id : '',
                verbaiterName: selectedVerbaiter ? selectedVerbaiter.username : '',
                verbastard: value,
                verbastardName: verbastardUsername, // Update the verbastardName property with the verbastard username
              });
              ...
              console.log('Verbastard not found with the selected verbastard id.');
            }
          });
      } 
    } else {
